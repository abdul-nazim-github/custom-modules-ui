...
export async function PUT(
  ...
  try {
    ...
    console.log(`API Route: /api/auth/users/${id}/permissions - accessToken present:`, !!accessToken);
    if (accessToken) {
      console.log(`API Route: /api/auth/users/${id}/permissions - Token starts with: ${accessToken.substring(0, 10)}...`);
    }

    if (!accessToken) {
      console.error(`API Route: /api/auth/users/${id}/permissions - No access token found in cookies`);
